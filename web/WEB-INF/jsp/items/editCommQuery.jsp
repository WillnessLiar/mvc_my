...
<head>
    ...
    <script type="text/javascript">
        function editCommAllSubmit() {
            document.commForm.jsp = "${pageContext.request.contextPath }/Comm/editCommAllSubmit"
    ...
    <form name="commForm" action="${pageContext.request.contextPath }/Comm/queryComm" method="post">
        <table>
            <tr>
                ...
                <td>商品名称:<input name="commCustom.comm_name"/></td>
                <td><input type="button" value="搜索" onclick="queryComm()"
                <%--class="form-control" id="strValue"  placeholder="搜索编号或名称"--%>>
                    <input type="button" value="批量修改提交" onclick="editCommAllSubmit()"/>
                </td>
    ...
    <table class="table table-responsive table-bordered" id="commList" width="100%" border=1>
        ...
        <c:forEach items="${commList}" var="comm" varStatus="status">
            <tr>
                ...
                <td><input name="commCustomList[${status.index}].comm_id" class="comm_Id" value="${comm.comm_id}"/></td>
                <td><input name="commCustomList[${status.index}].comm_name" class="comm_Name"value="${comm.comm_name}"/></td>
                <td><input name="commCustomList[${status.index}].comm_price" class="comm_Price" value="${comm.comm_price}"/></td>
                <td><input name="commCustomList[${status.index}].date" class="date" value="<fmt:formatDate value="${comm.date}" pattern="yyyy-MM-dd"/>" /></td>
